...
def leftRight(curname, newname):
    if len(curname) < len(newname):
        maxLen = len(curname)
    else:
        maxLen = len(newname)
...
class PrefNode:
    ...
    def searchChildren(self, cname):
        ...
        if(leftRight(self.name,cname)) == "left":
            if self.leftChild:
                return self.leftChild.searchChildren(cname)
            return None
        if(leftRight(self.name,cname)) == "right":
            if self.rightChild:
                return self.rightChild.searchChildren(cname)
            return None
        return None
    ...
    def addChild(self, child, v=False):
        ...
        if self.name == child.name:
            return False
        if(leftRight(self.name,child.name)) == "left":
            if self.leftChild:
                return self.leftChild.addChild(child)
            self.leftChild = child
            return True
        if(leftRight(self.name,child.name)) == "right":
            if self.rightChild:
                return self.rightChild.addChild(child)
    ...
    def findAllSim(self, cname, N=7):
        ...
        top = self.findSimilar(cname, N, top)
    ...
    def findSimilar(self, cname, N, top):
        top = addResult(self.name, cname, top, N)
        if self.leftChild:
            top = self.leftChild.findSimilar(cname, N, top)
        if self.rightChild:
            top = self.rightChild.findSimilar(cname, N, top)
        return top
